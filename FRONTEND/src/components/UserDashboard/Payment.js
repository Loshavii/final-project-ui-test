...
const CheckoutForm = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [cardholderName, setCardholderName] = useState('');
  const [amount] = useState(800); // Fixed amount for video option: $8.00
  const [packageDetails, setPackageDetails] = useState({ name: 'Video Consultation', description: 'A one-time video consultation.' });
    ...
    try {
      ...
      if (error) {
        ...
        if (userEmail) {
          ...
          emailjs
            .send('service_sdz5ece', 'template_q9ja6b6', templateParams, '6YJpNZJnOKv4hOLAn')
            .then((result) => {
              console.log('Email sent successfully:', result.text);
            })
            .catch((error) => {
